...
def load_data(messages_filepath, categories_filepath):
    ...
    logging.info(f'Messages and categories combined on {merge_col}. Dims: {df.shape}.')
    return df
...
def clean_data(df):
    ...
    for column in categories:
        ...
        categories[column] = pd.to_numeric(categories[column].str.split('-').str[-1])
...
def save_data(df, database_filename='messages.db', table_name='messages'):
    ...
    logging.info(f'Saved to {database_filename}.')
...
def main():
    ...
    parser.add_argument('messages_filepath', help='messages data as a csv, e.g. messages.csv')
    parser.add_argument('categories_filepath', help='categories data as a csv, e.g. categories.csv')
    parser.add_argument('database_filepath', help='path to save the output database, e.g. disaster_response.db')
    # Get sys args
